 ...
 * GET /api/v1/videographer-reports/by-date?dateString=YYYY-MM-DD&userId=...
 */
export const getReportByDate = async (req, res, next) => {
  try {
    ...
    if (targetUserId) {
      if (targetUserId !== String(currentUserId) && !isPrivileged && !isVideographer) {
    ...
    const report = await VideographerReport.findOne({
      userId: targetUserId,
 ...
 * POST /api/v1/videographer-reports
 */
...
/**
 * 3. GET VIDEOGRAPHERS LIST (For dropdown selection)
 * GET /api/v1/videographer-reports/videographers
 */
export const getVideographersList = async (req, res, next) => {
  try {
    ...
    console.error('Error in getVideographersList:', error);
    next(error);
  }
};
 ...
 * GET /api/v1/videographer-reports/submitted-dates?userId=...
 */
export const getSubmittedDates = async (req, res, next) => {
  try {
    ...
    const reports = await VideographerReport.find({ userId: targetUserId }, 'dateString');
