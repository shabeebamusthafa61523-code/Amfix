...
const PersonalPayslipPage = () => {
  ...
  // ── Filter: STACK-ENFORCED personal payslips only ──────────────
  ...
  const filteredRecords = useMemo(() => {
    return myRecords.filter(r => {
      ...
      const matchesSearch = !q || month.includes(q) || payPeriod.includes(q);
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight flex items-center gap-2">
              Personal Payslips
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 border border-indigo-200/60 dark:border-indigo-800/60">
                <ShieldCheck size={11} /> Confidential
              </span>
            </h1>
            <p className="text-xs font-bold text-slate-500 dark:text-slate-400 mt-0.5">
              View and download your official monthly salary disbursal statements ({user?.name || 'Employee'})
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xs">
        ...
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            placeholder="Search pay period, month..."
      ...
      {loading ? (
        <div className="py-20 text-center text-slate-400 font-medium">
          ...
          Loading your personal payslips...
        </div>
            ...
            return (
              ...
              >
                <div className="space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-extrabold text-sm text-slate-900 dark:text-white">{r.month}</h3>
                        <p className="text-[11px] text-slate-400 font-medium">Personal Salary Disbursal</p>
                      </div>
                    </div>
                  ...
                  <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div>
                      <span className="block text-[10px] text-slate-400 font-bold uppercase">Payment Date</span>
                ...
                <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  <span className="text-[11px] text-slate-400 font-medium">Personal Slip #{r.kbEmployeeId || r._id.slice(-6)}</span>
                  <button
...
export default PersonalPayslipPage;
